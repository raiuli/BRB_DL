...
fid = fopen ('Dataset/PUE_FB_old.csv', 'r');
...
%indices = crossvalind('Kfold',allvalueSet(:,5),10);
%ffilename = strcat('Dataset/JISC_Dataset_Paper_refined-2_small100_cr10','_indices_','.csv');
...
%     brbTree(1).conRefval=[1.254 1.127 1];
    ...
    brbTree(1).antRefval={[24 12 0];
       [20 10 0]
       };
    brbTree(1).consequent=cellstr('x4');
    brbTree(1).conRefval=[86 43 0];
    ...
    for brdTreeID=1:size(brbTree,2)
        ...
        formatOut = 'FFF';
        tS=datetime('now');
        f=objFunAllParallelDisv2(x0,brbConfigdata);
        tE=datetime('now');
        datestr(tE-tS,formatOut)
        fprintf (fid_x1,'\nF=%d\n',f);
        fprintf ('\nF=%d\n',f);
        ...
        for i=1:1
            %v(i)= rng(i);
            %v(i)=rng();
            [x,f,nf] = BRBaDEv6('objFunAllParallelDisv2',VTR,D,XVmin,XVmax,y,NP,itermax,F,CR,strategy,refresh,brbConfigdata,i)
            trainparameter(i,:)=x';
            fvalue(i)=f;
            nofof(i)=nf;
        end
        ...
        %while all(oldBestval-oldBestval(1)) && (lenght(oldBestval)==10)
            ...
            %newbrbStructSet= StructureOptimizationv1dis(brbStructSets,brbParaSets,brbConfigdatas);
            newbrbStructSet.antecedent=brbStructSets(1).antecedent;
            for c=1:it
                new_rf=ones(1,c)*99;
            end
            for i=1:size(brbStructSets(1).antRefval,1)
                newbrbStructSet.antRefval(i,:)={new_rf};
            end 
            newbrbStructSet.consequent=brbStructSets(1).consequent;
            newbrbStructSet.conRefval=brbStructSets(1).conRefval;
            ...
            %if j_t<=j_k
                ...
                [lb,ub,brbConfigdata]=UpdateParametersv1dis(brbConfigdata,fid_x1,lbCU,ubCU);
                XVmin = lb;
                XVmax = ub;
...
%                     itermax=5000;
%                 end    
                [x,f,nf] = BRBaDEv6('objFunAllParallelDisv1',VTR,D,XVmin,XVmax,y,NP,itermax,F,CR,strategy,refresh,brbConfigdata,1)
...
%                     brbConfigdatas(counter)=brbConfigdata;
        ...
        observedOutput=cell2mat(valueSet(find(strcmp(keySet,brbTree(brdTreeID).consequent{1}))));
        best_brbConfigdata.input=in;
        best_brbConfigdata.observedOutput=observedOutput;
        best_brbConfigdata.transformedRefVal=transformedRefVal;
        best_brbConfigdata.sizeOfData=sizeOfData;
        ...
        [f,outputOpti]=objFunAlleva4dis1(best_params,best_brbConfigdata);
        fprintf (fid_x1,'\nActual Output vals:');
