...
while ((iter < itermax) & (bestval > VTR))
    ...
    if (iter>1 )
        ...
        if iter>=200
            if (length(oldBestval)>502)&& (bestval > VTR)
                if (sum(oldBestval(length(oldBestval)-502:end)-oldBestval(end))==0)
                    %newF=0.8;
                    %newCR=0.8;
                    itermax=iter;
                end
...
%     end
    ...
    for i=1:NP 
        ...
        %nfeval  = nfeval + 1;
        currentBest=min(tempval(i),currentBest);
    ...
    if (refresh > 0)
        if (rem(iter,refresh) == 0)
            %xplt(NP,pop,0,0)
            fprintf(1,'Iteration: %d,  Best: %f, CuBest: %f, F: %f,  CR: %f,  NP: %d\n',iter,bestval,currentBest,F,CR,NP);
        end
        fprintf(fid_x3,'Iteration: %d,  Best: %f, CuBest: %f,  F: %f,  CR: %f,  NP: %d\n',iter,bestval,currentBest,F,CR,NP);
    end
